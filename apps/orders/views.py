...
def _get_seller_order(order_number: str, user) -> Order | None:
    """Returns an order for the seller's market or None."""
    try:
...
def _get_order_any(order_number: str, user) -> Order | None:
    ...
    try:
        return Order.objects.get(Q(order_number=order_number) & Q(user=user) | Q(market__seller=user))
    except Order.DoesNotExist:
        return None
...
class CheckoutView(APIView):
    ...
    Creates one order per market from the user's cart.
    ...
    Response:
    {
        ...
        "data": {
            "order_count": 2,
            "orders": [ ...OrderDetailSerializer for each order... ],
            "total_charged": "149.97"
        }
    }
    """
    ...
    def post(self, request):
        from .serializers import OrderCreateSerializer as MultiMarketOrderCreateSerializer
        from decimal import Decimal

        serializer = MultiMarketOrderCreateSerializer(data=request.data, context={"request": request})
        serializer.is_valid(raise_exception=True)
        orders = serializer.save()

        total_charged = sum(o.total_amount for o in orders).quantize(Decimal('0.01'))
        return Response(
            {
                "status": "Success",
                ...
                "data": {
                    ...
                    "next_step":        "Pay for each order at POST /payments/pay/",
                },
            },
...
class OrderListView(APIView):
    ...
    Buyer: own orders.
    Support ?status=, ?payment_status= query filters.
    """
    permission_classes = [IsAuthenticated]
...
class OrderDetailView(APIView):
    ...
    def delet(self, request, order_number):
        order = _get_buyer_order(order_number, request.user)
        if not order:
            return not_found("Order not found.")
...
class OrderTrackingView(APIView):
    ...
    Buyer-facing tracking page - not auth required for public tracking links.
    """
    permission_classes = [IsAuthenticated]
...
class OrderSearchView(APIView):
    ...
    def get(self, request):
        ...
        qs = qs.filter(
            Q(order_number__icontains=q) | Q(order_items__product_name__icontains=q) | Q(order_items__sku__icontains=q)
...
class OrderShippingView(APIView):
    """
    GET     /orders/<order_number>/shipping     -> shiping details
...
class ShippingMethodListCreateView(APIView):
    ...
    def get(self, request):
        ...
        if not market_id:
            return Response(
                {"status": "error", "message": "market_id query params is required."},
                status=status.HTTP_400_BAD_REQUEST,
            )
