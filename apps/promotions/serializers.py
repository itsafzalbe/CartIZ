...
==========================
Coupons         (11) CouponSerializer, CouponCreateSerializer, CouponUpdateSerializer,
                     CouponDeleteSerializer, CouponListSerializer, CouponDetailSerializer,
                     CouponValidateSerializer, CouponApplySerializer, CouponPublicSerializer,
                     CouponStatsSerializer, CouponActiveSerializer

Coupon Usage    (3)  CouponUsageSerializer, CouponUsageListSerializer,
                     CouponUsageStatsSerializer

Deals & Sales   (10) DealOfTheDaySerializer, FlashSaleSerializer,
                     FlashSaleCreateSerializer, FlashSaleUpdateSerializer,
                     ClearanceSaleSerializer, SeasonalSaleSerializer,
                     BundleDealsSerializer, ActiveDealsSerializer,
                     UpcomingDealsSerializer, ExpiredDealsSerializer

Promotions      (6)  PromotionSerializer, PromotionCreateSerializer,
                     PromotionUpdateSerializer, PromotionListSerializer,
                     PromotionDetailSerializer, PromotionStatsSerializer
"""
...
def _primary_image(product: Product) -> str | None:
    img = product.product_images.filter(is_primary=True).first() \
          or product.product_images.first()
...
class CouponUsageStatsSerializer(serializers.Serializer):
    ...
    def to_representation(self, queryset):
        ...
        top_coupons = (
            queryset.values('coupon__code')
            .annotate(uses=Count('id'), saved=Sum('discount_amount'))
            .order_by('-uses')[:5]
        )
        return {
            'total_uses':      agg['total_uses']      or 0,
...
class DealOfTheDaySerializer(serializers.ModelSerializer):
    """
    Current deal of the day — the active flash sale with the highest discount.
...
class PromotionUpdateSerializer(serializers.ModelSerializer):
    """
    Updates promotion details. Seller only.
    PATCH /promotions/<id>/
    """
    product_ids = serializers.ListField(child=serializers.IntegerField(), required=False)
