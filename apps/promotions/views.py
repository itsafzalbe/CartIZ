...
====================
Permissions:
  AllowAny       – public coupons, deals, promotions
  IsAuthenticated– validate / apply coupon, view usage history
  IsSellerOnly   – create/update/delete own coupons, flash sales, promotions
  IsAdminUser    – platform-wide promotions
"""
...
class CouponStatsView(APIView):
    ...
    def get(self, request, pk):
        ...
        if not _owns_coupon(request.user, coupon):
            return forbidden()
        return ok("Coupon stats retrieved.", data=CouponStatsSerializer(coupon).data)
...
class CouponActiveListView(APIView):
    """
    GET /promotions/coupons/active/?market_id=<id>
    Currently active coupons — used in cart page dropdown.
    """
    permission_classes = [IsAuthenticated]
...
class DealOfTheDayView(APIView):
    ...
    def get(self, request):
        ...
        return ok("Deal of the day retrieved.", data=DealOfTheDaySerializer(deal).data)


class ActiveDealsView(APIView):
    """GET /promotions/active/ — all currently live promotions."""
    permission_classes = [AllowAny]
...
class PromotionClickView(APIView):
    ...
    def post(self, request, pk):
        Promotion.objects.filter(pk=pk).update(
            click_count=Promotion.objects.get(pk=pk).click_count + 1
        )
